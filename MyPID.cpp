 ...
 * integral and differental terms. The default is 50ms.
 */
MyPID::MyPID(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period) :
	m_semaphore (0)
{
...
void MyPID::Initialize(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period)
{
	m_table = NULL;
...
void MyPID::Calculate()
{
	...
	if (enabled)
	{
...

		{
			...
			m_result += m_D * (m_error - m_prevError);
			m_prevError = m_error;
 ...
 */
void MyPID::SetPID(float p, float i, float d)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void MyPID::SetPID(float p, float i, float d, float f)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void MyPID::SetContinuous(bool continuous)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void MyPID::SetInputRange(float minimumInput, float maximumInput)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void MyPID::SetOutputRange(float minimumOutput, float maximumOutput)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Set the setpoint for the MyPID
...
/**
 * Returns the current setpoint of the MyPID
 * @return the current setpoint
 */
float MyPID::GetSetpoint()
{
	float setpoint;
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void MyPID::SetTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 * @param percentage error which is tolerable
 */
void MyPID::SetPercentTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_toleranceType = kPercentTolerance;
 ...
 * @param percentage error which is tolerable
 */
void MyPID::SetAbsoluteTolerance(float absTolerance)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Stop running the MyPID, this sets the output to zero before stopping.
 */
void MyPID::Disable()
{
	CRITICAL_REGION(m_semaphore)
...
/**
 * Return true if MyPID is enabled.
 */
...
std::string MyPID::GetSmartDashboardType(){
	return "PIDController";
}

void MyPID::InitTable(ITable* table){
	if(m_table!=NULL)
...
ITable* MyPID::GetTable(){
	return m_table;
}

void MyPID::ValueChanged(ITable* source, const std::string& key, EntryValue value, bool isNew){
...
void MyPID::UpdateTable() {
	
}

void MyPID::StartLiveWindowMode() {
	Disable();
}

void MyPID::StopLiveWindowMode() {
